  ...
  return (
    <div className="p-6 bg-gray-100 min-h-screen">
      ...
      <div className="flex justify-center gap-4 mb-6">
        ...
        <button
          className={`px-4 py-2 rounded ${selectedAnomaly === 'Medium' ? 'bg-yellow-500 text-white' : 'bg-white text-yellow-500 border border-yellow-500'}`}
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {selectedAnomaly &&
            findings[selectedAnomaly]?.map((anomaly) => (
              <div key={anomaly.id} className="bg-white p-4 rounded shadow-md">
                <h2 className="text-xl font-semibold mb-2">{anomaly.description || 'Unnamed Anomaly'}</h2>
                <p className="text-gray-600"><strong>Type:</strong> {anomaly.anomalyType}</p>
                <p className="text-gray-600"><strong>Category:</strong> {anomaly.category}</p>
                <p className="text-gray-600"><strong>Transaction ID:</strong> {anomaly.transactionId}</p>
          ...
          {(!findings[selectedAnomaly] || findings[selectedAnomaly].length === 0) && (
            <p className="text-center">No anomalies found for {selectedAnomaly} flag.</p>
          )}
        </div>
      )}
    </div>
